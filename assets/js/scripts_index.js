...
function index() { // Si le Launcher est à Jour
    ...
    function logged() {
        if (localStorage.getItem("accessToken") && localStorage.getItem("clientToken") && localStorage.getItem("selectedProfile")) {

            ipc.send("autoLogin", {
                aToken: localStorage.getItem("accessToken"),
                cToken: localStorage.getItem("clientToken"),
                profile: localStorage.getItem("selectedProfile")
            })

            // Doit renvoyer à ipc.on("autoUser", (...) => { ... }) ou à ipc.on("autoErr", (...) { ... })
...
ipc.on("autoErr", (event, err) => {
    localStorage.clear();
    console.error("%c[Launcher]" + "%c [Connexion auto]" + "%c Error with autoLogin" + err, "color: blue; font-weight: 1000", "color: black; font-weight: 700", "color: black; font-weight: 100");
})

function needUpdate() { // Si le launcher n'est pas à Jour

    if (os.platform == "win32") {
        updaterPath = os.homedir + "/AppData/Local/Temp/arakarahm-updater.exe"
    } else if (os.platform == "darwin") {
        updaterPath = "users/" + userName + "/Desktop/arakarahm-updater.dmg"
    } else if (os.platform == "linux") {
        updaterPath = "home/" + userName + "arakarahm-updater.appImage"
    }
...
taskkill /Im "Arak-Arahm Launcher.exe"
echo Launcher ferme.
echo Lancement du programme d'installation...
cd %temp%
start arakarahm-updater.exe
...
echo Fermeture de la console...`, (err) => {
                ...
                buttonFrom.innerHTML = "Vous semblez être sur macOS.<br>Veuillez fermer le Launcher puis ouvrir manuellement l'exécutable de Mise à Jour ; celui-ci se trouve sur votre Bureau (Desktop), et se nomme <i>arakarahm-updater.dmg</i>."
