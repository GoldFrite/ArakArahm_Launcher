...
ipcMain.on("login", (event, data) => {
  Authenticator.getAuth(data.user, data.password)
    .then((user) => {
...
ipcMain.on("autoLogin", (event, data) => {
  Authenticator.validate(data.aToken, data.cToken)
    .then((user) => {
      // mainWindow.webContents.send("autoUser", user)
      mainWindow.loadURL(path.join(__dirname, "app.html"))
    })
    .catch((err) => {
      mainWindow.webContents.send("autoErr", err)
      mainWindow.loadURL(path.join(__dirname, "login.html"))
    })
})
...
ipcMain.on("logout", (event, data) => {
  mainWindow.loadURL(path.join(__dirname, "index.html"))
    .then(() => {
      Authenticator.invalidate(data.aToken, data.cToken)
    })
    .catch(() => { })
})
